...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        for race in orm.Race.objects.all():
            if not race.ambassadors.exists():
                race.is_ai = True
            if race.official.exists():
                gamerace = race.official.get()
                race.official_racefile = gamerace.racefile
                gamerace.delete()
            race.save()

    def backwards(self, orm):
        for race in orm.Race.objects.all():
            if race.official_racefile:
                race.official.create(racefile=race.official_racefile)
